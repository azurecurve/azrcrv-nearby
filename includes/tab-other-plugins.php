<?php
/*
	other plugins tab on settings page - shared azurecurve pattern, listing
	every other (non-retired) azurecurve plugin the admin isn't already
	using, read from the same 'azrcrv-plugin-menu' option the shared
	cross-plugin menu itself is built from (see azurecurve-menu-populate.php
	and azurecurve-menu-display.php).
*/
...
$tab_plugins_label = esc_html__( 'Other Plugins', 'azrcrv-n' );
$tab_plugins        = '
<table class="form-table azrcrv-settings">
	...
	<tr>
		...
		<td scope="row" colspan=2>
			...
			<p>' .
				sprintf( esc_html__( '%1$s was one of the first plugin developers to start developing for ClassicPress; all plugins are available from %2$s and are integrated with the %3$s plugin for fully integrated, no hassle, updates.', 'azrcrv-n' ), '<strong>' . DEVELOPER_SHORTNAME . '</strong>', DEVELOPER_LINK, '<a href="https://directory.classicpress.net/plugins/update-manager/">Update Manager</a>' )
			. '</p>
			<p>' .
				sprintf( esc_html__( 'Other plugins available from %s, which you are not using, are:', 'azrcrv-n' ), '<strong>' . DEVELOPER_NAME . '</strong>' )
			. '</p>

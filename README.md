# [Nearby](https://development.azurecurve.co.uk/classicpress-plugins/nearby/)
![Plugin Banner](/assets/images/banner-1544x500.png)

Nearby creates a table of nearby locations based on GPS co-ordinates.
...
Example of this plugin in action:

* [DarkForge|Tourist Attractions](https://www.darkforge.co.uk/attractions/europe/republic-of-ireland/east/county-meath/newgrange-monument/)
...
### [Version 4.0.0](https://github.com/azurecurve/azrcrv-nearby/releases/tag/v4.0.0)

* Rewritten to the current azurecurve plugin structure (namespaced, split into includes/ files), matching Shortcodes in Comments.
* Nearby locations and distances are now precalculated and cached in a new database table, instead of being calculated live with every page view; a "Rebuild Distance Cache" button has been added to the Default Settings tab, and the cache is kept up to date automatically as locations are added, edited or removed.
* Removed the jQuery/jQuery UI dependency from the admin settings page; the tab UI is now vanilla JavaScript.
* Fixed a bug where a plain [nearby] shortcode (with no type attribute) could show no results at all on sites which had defined location types but not configured a site-wide default type list.
* Fixed a bug where the "in use" count on the Types tab always failed, allowing in-use types to be deleted without warning.
* Fixed a bug where locations without co-ordinates could still be included in distance calculations.
* Fixed the plugin text domain used by the admin menu labels so they can be translated.
* Fixed the plugin's uninstall routine, which previously did not remove the Types setting or either database table.
* Bumped minimum requirements to ClassicPress 1.0 and PHP 8.2.
* Update readme.md and remove readme.txt (not required for ClassicPress).
* Update azurecurve menu.
* The Flags integration (Location field on the co-ordinates metabox) and Icons integration (visited-icon setting) now show a visual picker of the actual flag/icon images to choose from, built by scanning the Flags/Icons plugins' own image folders, instead of a plain dropdown or a field where the id had to be typed by hand.
